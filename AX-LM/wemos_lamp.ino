...
// ==================== CONFIGURAÇÕES ====================
const char* apSSID = "AX-LM_CONFIG";        
...
void handleRoot() {
  ...
  <head>
    ...
    <title>AX-LM - Configuração WiFi</title>
    <style>
      * { 
        margin: 0; 
        padding: 0; 
    ...
    <div class="container">
      <div class="card">
        ...
        <div class="info-box">
          ...
          <p>4. O dispositivo irá reiniciar e controlar a AX-LM automaticamente</p>
        </div>
      ...
      <div class="footer">
        <p>Servidor: 192.168.4.1 | AXII Smart Lighting System</p>
      </div>
    </div>
  </body>
  </html>
...
String page = R"rawliteral(
  ...
  <body>
    <div class="box">
      ...
      <p>O AX-LM estará pronto para uso em instantes.</p>
    </div>
  </body>
  </html>
  )rawliteral";
...
void consultarDispositivo() {
  ...
  if (httpResponseCode == 200) {
      ...
      if (success) {
        ...
        if (ativo && status == "offline") {
          if (!releAtivo) {
            ...
            releAtivo = true;
            Serial.println(">>> RELÉ LIGADO - AX-LM ACESA <<<");
            atualizarUltimaConexao();
          } else {
            Serial.println(">>> RELÉ JÁ ESTÁ LIGADO <<<");
          }
        } else {
          if (releAtivo) {
            digitalWrite(pinoRele, LOW);
            releAtivo = false;
            Serial.println(">>> RELÉ DESLIGADO - AX-LM APAGADA <<<");
          } else {
            Serial.println(">>> RELÉ JÁ ESTÁ DESLIGADO <<<");
          }
        }
      } else {
    ...
    Serial.println("Dispositivo não encontrado (404)");
  }
  else {
...
void atualizarUltimaConexao() {
  ...
  if (httpResponseCode == 200) {
    Serial.println("Última conexão atualizada!");
  }
...
void setup() {
  ...
  Serial.println("\n\n========================================");
  Serial.println("    AXII - Controle de AX-LM");
  ...
  digitalWrite(pinoRele, LOW);
  Serial.println("Relé configurado - Estado: DESLIGADO\n");
